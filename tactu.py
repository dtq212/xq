...
class TacTu:
    ...
    def action_tudongtimkiemmuctieu(self):
        if self._is_tudongtimkiemmuctieu:
            ...
            if idbandohientai != self._idbandomuctieuduphong:
                self._idbandomuctieuduphong = idbandohientai
                self._diachicosomuctieuduphong = 0
            ...
            if self._idmuctieudangdichuyenkhaithien > 0:
                if time.time() - self._thoidiembatdaudichuyenkhaithien > 3.0:
                    ...
                    diachimuctieukhoa = self.moitruong.action_timkiemnhanvat(iddoituong = self._idmuctieudangdichuyenkhaithien)

                    if diachimuctieukhoa and not self.moitruong.get_is_nhanvatdachet(diachimuctieukhoa):
                        if self.moitruong.get_diachicosothongtinnhanvatmuctieudangchon() != diachimuctieukhoa:
                            self.moitruong.set_diachicosothongtinnhanvatmuctieudangchon(diachimuctieukhoa)
                        return
                    else:
                        self._idmuctieudangdichuyenkhaithien = 0
    ...
    def _action_sudungkynang_vanmongcoc(self):
        ...
        while True:
            ...
            while True:
                ...
                elif tendoituong := self.moitruong.get_tendoituong(diachicosothongtinnhanvatxemxet):
            ...
            if self._is_tudongkhaikhoang and not self.moitruong.get_is_cohieuungs((HIEUUNGKYNANG_LINHKHIHOTHE, ), macdinh = True, is_hieuungcoloi = 1) and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_LINHKHIHOTHE):
                self._is_tamngungdichuyensudungkynang = True
                if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() -  self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                    self.moitruong.action_sudungkynangvitrilenbanthan(*VITRIKYNANG_LINHKHIHOTHE)
                break
            ...
            if diachicosothongtinnhanvatmuctieudangchon:
                ...
                if thoigiantuthenhanvatdungim > 0.5 and khoangcach > KHOANGCACHSUDUNGKYNANGTAMXA - 3:
                    ...
                    if khoangcach <= KHOANGCACHSUDUNGKYNANGTAMXA:
                        ...
                        if phantramnoiluc > 25 and khoangcach <= KHOANGCACHSUDUNGKYNANGCANCHIEN and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_DONGIAPTRAN) and ("Trá»™m Báº£o" in self.moitruong.get_tendoituong(diachicosothongtinnhanvatmuctieudangchon) or self._is_nhieumuctieugan):
                            self._is_tamngungdichuyensudungkynang = True
                            if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() -  self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                                self.moitruong.action_sudungkynangvitriphudau(*VITRIKYNANG_DONGIAPTRAN, diachicosothongtinnhanvatmuctieudangchon, khoangcachphudau = khoangcach - random.randint(-2, 2))
                            break
                        if phantramnoiluc > 25 and khoangcach <= KHOANGCACHSUDUNGKYNANGCANCHIEN and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_KYMONTRAN) and ("Trá»™m Báº£o" in self.moitruong.get_tendoituong(diachicosothongtinnhanvatmuctieudangchon) or self._is_nhieumuctieugan):
                            self._is_tamngungdichuyensudungkynang = True
                            if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() -  self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                                self.moitruong.action_sudungkynangvitriphudau(*VITRIKYNANG_KYMONTRAN, diachicosothongtinnhanvatmuctieudangchon, khoangcachphudau = khoangcach - random.randint(-2, 2))
                            break
                        # if phantramnoiluc > 25 and not is_anhhuongboitruongnhom and not self.moitruong.get_is_cohieuungs((HIEUUNGKYNANG_CHAYMAU,), diachicosothongtinnhanvat = diachicosothongtinnhanvatmuctieudangchon, macdinh = True, is_hieuungcoloi = 0) and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_PHONGVUKINHTHIEN) and not self._is_nhieumuctieugan:
                        #     self._is_tamngungdichuyensudungkynang = True
                        #     if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() -  self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                        #         self.moitruong.action_sudungkynangvitrimuctieu(*VITRIKYNANG_PHONGVUKINHTHIEN)
                        #     break
                        if phantramnoiluc > 25 and not is_anhhuongboitruongnhom and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_LIETPHONGQUYET) and not self._is_nhieumuctieugan:
                            self._is_tamngungdichuyensudungkynang = True
                            if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() -  self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
    ...
    def _action_sudungkynang_maoson(self):
        ...
        while True:
            ...
            if phantramsinhlucconlai > 50 and noilucconlai <= 150 and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_HUYETMACHU):
            ...
            if noilucconlai > 150 and self.moitruong.get_iddoituongbaothumaoson():
            ...
            if self.moitruong.get_iddoituongbaothumaoson() and any(tenbaothuhiente in tendoituongbaothumaoson for tenbaothuhiente in (QUYTOT, THIENBINH)):
                if phantramsinhlucconlai <= 50:
                    if self.moitruong.get_is_dangnamtrongnhom():
                        if self.moitruong.get_is_kynangsansang(*VITRIKYNANG_MENHTE):
                            self._is_tamngungdichuyensudungkynang = True
                            if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() - self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                                self.moitruong.action_sudungkynangvitri(*VITRIKYNANG_MENHTE)
                            break
                    else:
                        if self.moitruong.get_is_kynangsansang(*VITRIKYNANG_HUYETTE):
                            self._is_tamngungdichuyensudungkynang = True
                            if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() - self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                                self.moitruong.action_sudungkynangvitri(*VITRIKYNANG_HUYETTE)
                            break
            else:
                if noilucconlai > 150 and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_TRIEUHOIQUYTOT):
                    self._is_tamngungdichuyensudungkynang = True
                    if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() - self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                        self.moitruong.action_sudungkynangvitri(*VITRIKYNANG_TRIEUHOIQUYTOT, delay = 1.)
                    break

            if noilucconlai > 150 and diachicosothongtinnhanvatmuctieudangchon:
                khoangcach = self.moitruong.get_khoangcach(diachicosothongtinnhanvatmuctieudangchon)
                is_muctieudangchonlanguoichoi = self.moitruong.get_is_nguoichoi(diachicosothongtinnhanvatmuctieudangchon)
                thoigiantuthenhanvatdungim = time.time() - self.moitruong.get_thoidiemtuthenhanvatdungimcomuctieugannhat() if idtuthenhanvat == TUTHENHANVAT_DUNGIM else 0.
                ...
                if thoigiantuthenhanvatdungim > 0.5 and khoangcach > KHOANGCACHSUDUNGKYNANGTAMXA:
                    ...
                    if noilucconlai > 150 and khoangcach <= KHOANGCACHHIEUQUAKYNANGLOIDONGCUUTHIEN and thoigiantuthenhanvatkhongdichuyen > 3.0 and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_LOIDONGCUUTHIEN) and ("Trá»™m Báº£o" in self.moitruong.get_tendoituong(diachicosothongtinnhanvatmuctieudangchon) or self._is_nhieumuctieugan):
                        self._is_tamngungdichuyensudungkynang = True
                        if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() -  self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                            self.moitruong.action_sudungkynangvitri(*VITRIKYNANG_LOIDONGCUUTHIEN, delay = 1.)
                        break
                    if not is_anhhuongboitruongnhom and noilucconlai > 150 and khoangcach <= KHOANGCACHSUDUNGKYNANGTAMXA and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_PHUCMAQUYET):
                        self._is_tamngungdichuyensudungkynang = True
                        if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() -  self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                    ...
                    if noilucconlai > 25 and not is_anhhuongboitruongnhom and khoangcach <= KHOANGCACHSUDUNGKYNANGTAMXA:
                        ...
                        if self.moitruong.get_is_kynangsansang(*VITRIKYNANG_MAIHOACHAM):
                            ...
                            if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() -  self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                                ...
                                self.moitruong.action_sudungkynangvitrimuctieu(*VITRIKYNANG_PHUCMAQUYET)
                            break
                        if self.moitruong.get_is_kynangsansang(*VITRIKYNANG_THAUCOTDINH) and not self.moitruong.get_is_cohieuungs((HIEUUNGKYNANG_THAUCOTDINH, ), macdinh = False, diachicosothongtinnhanvat = diachicosothongtinnhanvatmuctieudangchon, is_hieuungcoloi = 0):
                            self._is_tamngungdichuyensudungkynang = True
                            if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() -  self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                                print("thau cot dinh")
        ...
        while True:
            ...
            if diachicosothongtinnhanvatmuctieudangchon:
                ...
                if nguyenkhiconlai < 4 and self.moitruong.get_phantramsinhlucconlai() <= 65 and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_LUANHOIVANCHUYEN):
                    self._is_tamngungdichuyensudungkynang = True
                    self.moitruong.action_sudungkynangvitrimuctieu(*VITRIKYNANG_LUANHOIVANCHUYEN)
                    break
    ...
    def _action_sudungkynang_thucsondao(self):
        ...
        while True:
            ...
            is_bandocuthudao = self.moitruong.get_idbandohientai() in BANDOCUTHUDAOs if hasattr(self, 'BANDO_CUTHUDAOs') else (self.moitruong.get_idbandohientai() == 549)
            ...
            if self.moitruong.get_idbandohientai() not in BANDOKHONGTANCONGs and phantramsinhlucconlai <= 25 or (is_muctieudangchonlanguoichoi and phantramsinhlucconlai <= 50):
                if self.moitruong.get_is_kynangsansang(*VITRIKYNANG_TIENTHANVODICH):
                    ...
                    self.moitruong.action_sudungkynangvitri(*VITRIKYNANG_TIENTHANVODICH)
                    break
            if is_bandocuthudao and is_muctieudangchonlanguoichoi and time.time() - self._thoidiemsudungphihanhphugannhat > 1. and self.moitruong.get_phantramsinhlucconlai(diachicosothongtinnhanvatmuctieudangchon) > 50 and self.moitruong.get_is_cohieuungs((HIEUUNGKYNANG_TIENTHANVODICH, HIEUUNGKYNANG_KIMCUONGBATHOAIDON,), macdinh = False, diachicosothongtinnhanvat = diachicosothongtinnhanvatmuctieudangchon, is_hieuungcoloi = 1):
                self._thoidiemsudungphihanhphugannhat = time.time()
                self.action_sudungvatphamhanhtrang(PHIHANHPHU)
                break
            ...
            if diachicosothongtinnhanvatmuctieudangchon and is_cothetancong:
                ...
                if self._solanthatbaikhaithien >= 3:
                    if time.time() - self._thoidiembiphatkhaithien > 3.0:
                        self._solanthatbaikhaithien = 0
                        self._thoidiembiphatkhaithien = 0

                if not is_khaithientichdiasansang:
                    self._solanthatbaikhaithien = 0
                    self._thoidiembiphatkhaithien = 0.
                ...
                if khoangcach <= KHOANGCACHSUDUNGKYNANGTAMXA and is_luutinhtruymangsansang:
                    ...
                    if self.moitruong.get_is_cohieuungs(HIEUUNGBATLOITHUCSONCOTHEGIAIs, macdinh = False, is_hieuungcoloi = 0) and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_TINHTAMQUYET):
                        ...
                        self.moitruong.action_sudungkynangvitrimuctieu(*VITRIKYNANG_PHAKHONGKICH)
                        break
                    elif not is_muctieudangchonbichoang and self.moitruong.get_is_cothegaychoang(diachicosothongtinnhanvatmuctieudangchon) and is_muctieudangchonlanguoichoi and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_LUCPHACHHOASON):
                        self._is_tamngungdichuyensudungkynang = True
                        self.moitruong.action_sudungkynangvitrimuctieu(*VITRIKYNANG_LUCPHACHHOASON)
                        break
                ...
                if is_sudungkhaithientichdiaantoan and is_duocphepsudungkhaithientichdia and khoangcach <= 6.5:
                    SAFE_X = 4.2
                    SAFE_Y = 3.1
                    CAST_LIMIT_X = 6.2
                    CAST_LIMIT_Y = 4.7
                ...
                elif not is_khaithientichdiasansang or self._solanthatbaikhaithien >= 3:
                    if khoangcach > 3.0 and khoangcach <= KHOANGCACHSUDUNGKYNANGTAMXA and is_muctieudangchonlanguoichoi:
                        if not is_muctieudangchonbichoang and self.moitruong.get_is_cothegaychoang(diachicosothongtinnhanvatmuctieudangchon) and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_LANGKHONGCHIHUYET):
                            ...
                            if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() -  self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                                self.moitruong.action_sudungkynangvitrimuctieu(*VITRIKYNANG_LANGKHONGCHIHUYET)
                            break
                        elif not is_muctieudangchonbichoang and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_VANKIEMXUYENTAM):
                            self._is_tamngungdichuyensudungkynang = True
                            if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_SUDUNGKYNANGPHUTRO, TUTHENHANVAT_DELAYSAUTANCONG) and time.time() -  self.moitruong.get_thoidiemtuthenhanvatkhongdichuyen() > 0.25:
                                self.moitruong.action_sudungkynangvitrimuctieu(*VITRIKYNANG_VANKIEMXUYENTAM)
                            break
    ...
    def _action_nhatdo(self):
        ...
        if self._diachicosovatphamdangnhat:
            if not self.moitruong.get_is_vatphamtontai(self._diachicosovatphamdangnhat):
                self._diachicosovatphamdangnhat = False
            elif time.time() - self._thoidiemthaydoivatphamdangnhatgannhat > 10.0:
        ...
        if not self._diachicosovatphamdangnhat:
            ...
            while True:
                ...
                if is_cannhat:
                    kc = self.moitruong.get_khoangcach(diachivatpham)
                    if kc <= KHOANGCACHTOANMANHINH:
                        danhsachvatpham.append((kc, diachivatpham))
    ...
    def _action_dichuyentudo(self):
        ...
        while True:
            ...
            if diemdanhxungquanhbandos:
                ...
                elif self._diemdanhxungquanhhientai not in diemdanhxungquanhbandos:
                    is_cantimdiemgannhat = True
    ...
    def action_tudongkhaikhoang(self):
        ...
        if self._is_tamngungdichuyensudungkynang: return
        if not self.moitruong.get_is_cohieuungs((HIEUUNGKYNANG_LINHKHIHOTHE,), macdinh = False, is_hieuungcoloi = True): return
        ...
        while True:
            ...
            if tendoituong and ("KhoÃ¡ng" in tendoituong or "Khoáng" in tendoituong):
                iddoituong = self.moitruong.get_iddoituong(diachidoituong)
                if iddoituong <= 0: continue

                if time.time() - self._idkhoangbiloi_map.get(iddoituong, 0) < 120:
                    continue
        ...
        if diachidoituonggannhat:
            iddoituong = self.moitruong.get_iddoituong(diachidoituonggannhat)

            if iddoituong != self._idkhoangdangtheo:
                self._idkhoangdangtheo = iddoituong
                self._thoidiembatdautheokhoang = time.time()
            else:
                if time.time() - self._thoidiembatdautheokhoang > 7.0:
                    phatam("Khoáng vật bị kẹt, bỏ qua")
                    self._idkhoangbiloi_map[iddoituong] = time.time()
                    self._idkhoangdangtheo = 0
                    return

            self._diachicosokhaikhoang = diachidoituonggannhat

            if khoangcachgannhat >= 6:
                x_khoang = self.moitruong.get_toadox(diachidoituonggannhat, is_vitrihientai = True)
                y_khoang = self.moitruong.get_toadoy(diachidoituonggannhat, is_vitrihientai = True)

                self._yeucaukhaikhoang = {
                    "yeucau": 99,
                    "kieudichuyen": KIEUDICHUYEN_GIUKHOANGCACHTOIDA,
                    "toadodich": (x_khoang, y_khoang),
                    "khoangcachtoida": 0
                }
            else:
    ...
    def action_xulygomquai(self):
        ...
        i = -1
        so_quai_da_gom = 0
        id_quai_tiem_nang_gan_nhat = 0
        kc_quai_tiem_nang = 9999.
        diachi_quai_tiem_nang = 0

        has_mob_nearby = False

        while True:
            i += 1
            dc = self.moitruong.get_diachicosothongtindoituongx(i)
            if not dc: break

            if not self.moitruong.get_is_cothetancong(dc): continue
            if self.moitruong.get_idloainhanvat(dc) != LOAIMUCTIEU_QUAIVATHOACNPC: continue

            id_quai = self.moitruong.get_iddoituong(dc)
            kc = self.moitruong.get_khoangcach(dc)
            ...
            if kc <= 4.5:
                so_quai_da_gom += 1
                self._danhsachidquaidagom.add(id_quai)
                continue

            if kc <= 12.0 and id_quai not in self._danhsachidquaidagom:
                if kc < kc_quai_tiem_nang:
                    kc_quai_tiem_nang = kc
                    id_quai_tiem_nang_gan_nhat = id_quai
                    diachi_quai_tiem_nang = dc

        if not has_mob_nearby:
            self._danhsachidquaidagom.clear()

        if so_quai_da_gom >= self._soluongquaigomtoithieu or (id_quai_tiem_nang_gan_nhat == 0 and so_quai_da_gom > 0):
            self._is_danggomquai = False
            if so_quai_da_gom == 0:
                self._danhsachidquaidagom.clear()
            return
        ...
        if id_quai_tiem_nang_gan_nhat > 0 and diachi_quai_tiem_nang:
            if kc_quai_tiem_nang <= 2.5:
                self._danhsachidquaidagom.add(id_quai_tiem_nang_gan_nhat)
            else:
                x_quai = self.moitruong.get_toadox(diachi_quai_tiem_nang, is_vitrihientai = True)
                y_quai = self.moitruong.get_toadoy(diachi_quai_tiem_nang, is_vitrihientai = True)
