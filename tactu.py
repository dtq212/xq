            ...
            while True:
                ...
                if khoangcach <= KHOANGCACHSUDUNGKYNANGTAMXA:
                    if not self.moitruong.get_is_cohieuungs((HIEUUNGKYNANG_TRAMMAC,), False, diachicosothongtinnhanvat = diachicosothongtinnhanvatmuctieudangchon, is_hieuungcoloi = 0) and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_PHONGANTHUAT):
                        ...
                        is_tamngungdichuyensudungkynang = True
                        if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_DELAYSAUTANCONG):
                            self.moitruong.action_sudungkynangvitriphudau(*VITRIKYNANG_DONGIAPTRAN, diachicosothongtinnhanvatmuctieudangchon, khoangcachphudau = khoangcach + random.randint(-2, 1))

                    elif self.moitruong.get_is_kynangsansang(*VITRIKYNANG_KYMONTRAN):
                        is_tamngungdichuyensudungkynang = True
                        if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_DELAYSAUTANCONG):
                            self.moitruong.action_sudungkynangvitriphudau(*VITRIKYNANG_KYMONTRAN, diachicosothongtinnhanvatmuctieudangchon, khoangcachphudau = khoangcach + random.randint(-2, 1))
                            break
    ...
    def action_tudongsudungkynang_thucson(self):
        ...
        if self._is_tudongsudungkynang:
            while True:
                ...
                if time.time() - self._thoidiemsudungphihanhphugannhat > 1. and phantramsinhlucconlai <= 25. and not self.moitruong.get_is_kynangsansang(*VITRIKYNANG_TIENTHANVODICH) and self.moitruong.get_thoigianconlaihieuungtienthanvodich(macdinh = 2.5) < 2.:
                    self._thoidiemsudungphihanhphugannhat = time.time()
                ...
                if not diachicosothongtinnhanvatbatquaitran or self.moitruong.get_khoangcach(diachicosothongtinnhanvatbatquaitran) > 4.:
                    ...
                    if iddoituong:
                        if self.moitruong.get_is_dangnamtrongnhom() and self.moitruong.get_is_truongnhom():
                            is_ok = self.moitruong.action_thucthicaulenh_talk("talk {}# welcome.1".format(hex(iddoituong)).replace("0x", ""))
                            if is_ok:
                                time.sleep(2.5)
                                is_ok = self.moitruong.action_thucthicaulenh_talk("talk {}# welcome.11".format(hex(iddoituong)).replace("0x", ""))
                                if is_ok:
                                    time.sleep(0.25)
        ...
        if idbandohientai == BANDO_CHU:
            if self.moitruong.get_khoangcachdiem(X_SUGIAMONPHAI_CHU, Y_SUGIAMONPHAI_CHU) < self.moitruong.get_khoangcachdiem(X_PHITACDOATNGANTRUYENTONG_CHU, Y_PHITACDOATNGANTRUYENTONG_CHU):
                ...
                if not diachicosothongtinnhanvatsugiamonphai or self.moitruong.get_khoangcach(diachicosothongtinnhanvatsugiamonphai) > 6.:
                    ...
                    if iddoituong:
                        is_ok = self.moitruong.action_thucthicaulenh_talk("talk {}# 1".format(hex(iddoituong)).replace("0x", ""))
                        if is_ok:
                            time.sleep(0.25)
                ...
                if not diachicosothongtinnhanvatphitacdoatngantruyentong or self.moitruong.get_khoangcach(diachicosothongtinnhanvatphitacdoatngantruyentong) > 6.:
                    ...
                    if iddoituong:
                        ...
                        for caulenh in DICHUYENPHITACDOATNGUYENTRUYENTONG_MAP[idbando]:
                            is_ok = self.moitruong.action_thucthicaulenh_talk(caulenh.format(hex(iddoituong)).replace("0x", ""))
                            if is_ok:
                                time.sleep(0.25)
                            else:
                                break
            ...
            while True:
                ...
                if self.moitruong.get_is_datrieuhoibaothu():
                    ...
                    if diachicosonhanvatbaothudautien and time.time() - self._thoidiemthietlapbaothuchodoigannhat > 2.:
                        is_ok = self.moitruong.action_thucthicaulenh_pet("pet {}# 3".format(hex(iddoituongbaothudautien)).replace("0x", ""), delay = 0.5)
                        if is_ok:
                            self._thoidiemthietlapbaothuchodoigannhat = time.time()
                    break

                if time.time() - self._thoidiemtudongtrieuhoibaothudautien > 1. and self.moitruong.get_idtuthenhanvat() == TUTHENHANVAT_DUNGIM:
                    is_ok = self.moitruong.action_thucthicaulenh_pet("pet {}# show".format(hex(iddoituongbaothudautien)).replace("0x", ""), delay = 0.5)
                    if is_ok:
                        self._thoidiemtudongtrieuhoibaothudautien = time.time()
                    break
