...
class TacTu:
                    ...
                    if not is_boquamuctieuhientai and vungdabichiemdongs:
                        ...
                        for vung in vungdabichiemdongs:
                            if math.dist((mx, my), (vung["x"], vung["y"])) <= 9.0 and dist_curr > math.dist((mx, my), (vung["x"], vung["y"])):
                                is_boquamuctieuhientai = True; break
                ...
                if self.moitruong.get_is_nhanvattontai(diachicosothongtinnhanvatmuctieuxemxet) and \
                   ...
                   self.moitruong.get_idnguoichoi(diachicosothongtinnhanvatmuctieuxemxet) not in NHANVATCUAMINHs:
                     ...
                     if self.moitruong.get_is_cohieuungs((HIEUUNGKYNANG_ANTHANTHUAT,), macdinh=False, diachicosothongtinnhanvat=diachicosothongtinnhanvatmuctieuxemxet, is_hieuungcoloi=1):
                         if dist_to_me <= KHOANGCACHTOIDAHOPLE:
    ...
    def action_xulyuutiensudungkynang(self, loaikynang, vitrikynang, diachimuctieu, khoangcachphudau = 0):
        if loaikynang == "sudungkynangkhongmuctieu":
            ...
            is_ok = self.moitruong.action_sudungtancongvatly(diachimuctieu)
            if is_ok:
    ...
    def _action_tancong_duongmon(self):
        ...
        danhsachuutien = [
            (None, "dichuyengiukhoangcach", lambda: not is_bidaohoalacanh and diachimuctieu and (time.time() - self._thoidiemsudungamkichgannhat < 1. or time.time() - self._thoidiemsudungnhiephonchamgannhat < 1. or is_muctieukhongthedichuyen) and khoangcach <= KHOANGCACHSUDUNGKYNANGCANCHIEN + 1.5 and is_muctieupk, khoangcach + 3., None, False, True),
        ...
        for i, item in enumerate(danhsachuutien):
            ...
            if not diachimuctieu and loaikynang in ("sudungkynangmuctieu", "sudungkynangphudau", "tancongvatly", "dichuyentiepcantamxa", "dichuyentiepcancanchien", "dichuyengiukhoangcach"): continue
            ...
            if loaikynang == "dichuyengiukhoangcach":
                self._yeucautancong = {
                    "yeucau": YEUCAUDICHUYENTANCONG,
    ...
    def _action_tancong_vanmongcoc(self):
        ...
        for i, item in enumerate(danhsachuutien):
            ...
            if is_ok:
                ...
                self._thoidiemtamngungdichuyensudungkynang = max(self._thoidiemtamngungdichuyensudungkynang, time.time() + 0.8)
                self._yeucautancong = None
                if is_ketthucvonglap: return
            elif is_ketthucvonglap:
                return
    ...
    def action_tudongkhaikhoang(self):
        ...
        if idbandohientai == 81:
            ...
            idcanxoas = [k for k, v in self._idkhoangbiloi_map.items() if time.time() - v > 120]
            for k in idcanxoas:
