...
class TacTu:
        ...
        if self._is_tudongsudungkynang:
            while True:
                ...
                if is_duoitheo:
                    ...
                    elif self.moitruong.get_is_cohieuungbatloinhanvat() and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_TINHTAMQUYET):
                        is_tamngungdichuyensudungkynang = True
                        if idtuthenhanvat in (TUTHENHANVAT_DUNGIM, TUTHENHANVAT_TANCONG, TUTHENHANVAT_DELAYSAUTANCONG):
                    ...
                    elif khoangcach <= KHOANGCACHSUDUNGKYNANGCANCHIEN and is_muctieudangchonlanguoichoi and self.moitruong.get_is_cohieuungcoloinhanvat(diachicosothongtinnhanvatmuctieudangchon) and self.moitruong.get_is_kynangsansang(*VITRIKYNANG_PHAMATRAM):
    ...
    def action_tudongnhatdo(self):
        ...
        if self._is_tudongnhatdo:
            ...
            if not is_cododenhat and self.moitruong.get_idbandohientai() == BANDO_BATQUAITRAN:
                i = 0
        ...
        if idbandohientai == BANDO_CHU:
            if self.moitruong.get_khoangcachdiem(X_SUGIAMONPHAI_CHU, Y_SUGIAMONPHAI_CHU) < self.moitruong.get_khoangcachdiem(X_PHITACDOATNGANTRUYENTONG_CHU, Y_PHITACDOATNGANTRUYENTONG_CHU):
                ...
                if not diachicosothongtinnhanvatphitacdoatngantruyentong or self.moitruong.get_khoangcach(diachicosothongtinnhanvatphitacdoatngantruyentong) >= 4.:
                    self.moitruong.action_tudongtimduong(X_PHITACDOATNGANTRUYENTONG_CHU, Y_PHITACDOATNGANTRUYENTONG_CHU, BANDO_CHU)
